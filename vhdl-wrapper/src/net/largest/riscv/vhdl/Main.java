...
public class Main {
    ...
    private static String top_module_name = "RocketSystem";
    ...
    private static void generateRocketSystemDeclaration() {
        ...
        ln("");
        ln("    component " + top_module_name + " is");
        ln("    port (");
        ...
        ln("    end component " + top_module_name + ";");
    }
    ...
    private static void generateRocketSystemComponent() {
        ...
        ln("");
        ln("    rocket_system : component " + top_module_name);
        ln("    port map (");
    ...
    public static void main(String[] args) {
        int arg_pos = 0;
        while (arg_pos < args.length && args[arg_pos].startsWith("-")) {
            if (args[arg_pos].equals("-m")) {
                arg_pos++;
                if (arg_pos < args.length) rocket_module_name = args[arg_pos++];
            } else if (args[arg_pos].equals("-t")) {
                // Foxden: override the Verilog top-module name to search for.
                arg_pos++;
                if (arg_pos < args.length) top_module_name = args[arg_pos++];
            } else {
                break;
            }
        }
        if (args.length - arg_pos != 1) {
            ...
            System.err.println("    -m <name>   VHDL entity name to emit (default: rocket)");
            System.err.println("    -t <name>   Verilog top module to look for (default: RocketSystem)");
            System.exit(1);
        }
        try {
            ...
            parser.addParseListener(new Verilog2001BaseListener() {
                public void exitModule_declaration(Verilog2001Parser.Module_declarationContext ctx) {
                    ...
                    if (id != null) {
                        ...
                        if (name.equals(top_module_name)) {
                            rocket_system = ctx;
                            getBusSignals();
                        }
                    }
